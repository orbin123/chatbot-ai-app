...
export default function EditChatbot({params}: {
  ...
  const [addCharacteristic] = useMutation(
    ADD_CHARACTERISTIC, 
  {
  ...
  const [deleteChatbot] = useMutation(DELETE_CHATBOT, {
    refetchQueries: ["GetChatbotById"], // refetching the chatbot after deleting
  ...
  const handleUpdateChatbot =async (e:FormEvent<HTMLFormElement>) =>{
    ...
    try {
      ...
      toast.promise(promise, {
        loading: "Updating",
        success: "Chatbot successfully created",
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      console.error("Error deleting chatbots: ", error);
    ...
    <div className="px-0 md:p-10">
      <div className="md:sticky md:top-0 z-50 sm:max-w-sm ml-auto space-y-2 md:border p-5 rounded-b-lg md:rounded-lg bg-[#2991EE]">
        ...
        <p className="text-sm italic text-white">
          Share this link to start conversations with your chatbot.
        </p>
      ...
      <section className="relative mt-5 bg-white p-5 md:p-10 rounded-lg">
        ...
        <h2>Here's what your AI knows...</h2>
